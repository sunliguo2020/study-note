...
#采用HTTP POST方式发送检测信息给接口程序boobooke.php，接口程序负责分析信息，决定是否发送报警MSN消息
send_msg()
{
   /usr/bin/curl -m 10 -d menu=http -d date=$date -d ip=$server_ip -d port=$server_port -d status=$status http://127.0.0.1/boobooke.php 
}
