...
tar -C /www -cf ${workdictory}/blog_${date}.tar blog 
...
cd /var/sunliguo/wordpress
7zip a blog_${date}.tar.zip -psun@gao $workdictory/blog_${date}.tar
...
rm -f blog_${date}.tar wp_${date}.sql wp_${date}.sql
